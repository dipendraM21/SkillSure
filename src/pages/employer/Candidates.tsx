...
import { employerCandidateProfile } from '@/lib/utils/routes'
...
const EmployerCandidates = () => {
  return (
    <EmployerAppLayout>
      <ModuleProvider apiName={skillsureListingCrudApiNames.employerCandidates}>
        <EmployerCandidatesCrudBody />
      </ModuleProvider>
    </EmployerAppLayout>
  )
}

function EmployerCandidatesCrudBody() {
          ...
          <div className="min-w-[880px]">
            {rows.map((row, i) => (
              <CandidateListRow
                ...
                row={row}
                borderTop={i > 0}
                detailHref={employerCandidateProfile(row.id)}
              />
            ))}
          </div>
        </div>
