  ...
  },
  {
    logo: (
       <div className="flex items-center text-[#1E3A8A] font-bold text-[17px] tracking-tight whitespace-nowrap">
         <span className="w-6 h-6 mr-2 bg-blue-600 rounded flex items-center justify-center text-white text-[10px]">BC</span>
         BANSAL CLASSES
       </div>
    ),
  ...
  },
  {
    logo: (
      <div className="text-gray-900 font-black text-2xl tracking-tighter flex items-center">
        <span className="text-blue-600 mr-1">✦</span> NextGen
      </div>
    ),
...
export const TestimonialsSection = () => {
  ...
  return (
    <section className="w-full py-24 bg-white relative z-10 overflow-hidden" id="testimonials">
      <div className="max-w-[1280px] mx-auto px-6 flex flex-col items-center">
        
        {/* Header Badge */}
        <div className="flex items-center gap-2 px-4 py-1.5 rounded-full border border-gray-200 bg-white shadow-sm mb-6">
            <div className="w-2 h-2 rounded-full bg-blue-500"></div>
            <span className="text-sm font-medium text-blue-600">Wall of Love</span>
        </div>

        {/* Title & Description */}
        <h2 className="text-4xl md:text-5xl font-bold text-[#1a1a24] tracking-tight text-center mb-4">
          Trusted by Industry Leaders
        </h2>
        <p className="text-gray-500 text-lg max-w-2xl text-center mb-16 leading-relaxed">
          Join the founders and tech leaders who have transformed their platforms with our DevOps and scaling solutions.
        </p>
      </div>
      ...
      <div className="relative w-full overflow-hidden flex items-center py-4">
        ...
        >
          {duplicatedTestimonials.map((testimonial, idx) => (
            <div 
              key={idx} 
              className="flex flex-col bg-white rounded-3xl border border-gray-100 p-8 shadow-[0_4px_30px_rgba(0,0,0,0.03)] min-w-[380px] md:min-w-[480px] max-w-[480px] transition-transform hover:-translate-y-1 duration-300"
            >
